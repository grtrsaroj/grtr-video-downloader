...
export default function TikTok({ url }) {
  ...
  const fetchVideoFormats = useCallback(async () => {
    if (!url) return;
    try {
      const response = await axios.get('/api/tiktok', { params: { url } });
  ...
  const handleDownload = () => {
    if (selectedFormat) {
      window.location.href = `/api/downloadtt?url=${encodeURIComponent(url)}&format=${selectedFormat.itag}`;
    }
  };
  ...
  return (
    <div className="p-4">
      <div className="flex items-center gap-4 mb-4">
        ...
        <div>
          <h3 className="text-xl font-semibold mb-2">{videoTitle || 'TikTok Video Downloader'}</h3>
        ...
        <div className="flex flex-col gap-4">
          ...
          {selectedFormat && (
            <>
              ...
              {streamingUrl && (
                <Button onClick={handleStream} variant="secondary" className="w-full mt-2">
                  Stream
                </Button>
              )}
            </>
